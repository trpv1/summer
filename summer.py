...
task_indices = [i for i in range(5, 20) if contents.iloc[i].strip()]
total_tasks = len(task_indices)
completed_tasks = 0

for i in task_indices:
    title = titles.iloc[i].strip()
    content = contents.iloc[i].strip()
    key = f"{selected_date}_task_{i}"
    checked = st.checkbox(f"☑ {title}", key=key)
    if checked:
        completed_tasks += 1
    # 改行して内容表示
    st.markdown(f"<div style='margin-left:20px;'>{content}</div>", unsafe_allow_html=True)
...
try:
    ...
    announcement = contents.iloc[idx_renraku].strip()
    if announcement:
